 ...
 */
public abstract class BaseS3DynamoDBLogStore extends BaseExternalLogStore {
    private static final Logger LOG = LoggerFactory.getLogger(BaseS3DynamoDBLogStore.class);
    ...
    public BaseS3DynamoDBLogStore(Configuration hadoopConf) throws IOException {
        super(hadoopConf);
    ...
    protected void initClient(Configuration hadoopConf) throws IOException {
        client = getClient();
    ...
    private void tryEnsureTableExists(Configuration hadoopConf) throws IOException {
        ...
        while(retries < 20) {
            ...
            try {
                ...
                ProvisionedThroughput provisionedThroughput = getProvisionedThroughput(hadoopConf);
                LOG.info(
                        "DynamoDB table `{}` does not exist. " +
                                "Creating it now with provisioned throughput of {} RCUs and {} WCUs.",
                        tableName, provisionedThroughput.getReadCapacityUnits(), provisionedThroughput.getWriteCapacityUnits());
                try {
                    client.createTable(
